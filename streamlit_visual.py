...
st.title("🚌💩 Bus Departure Calculator")

st.write("Enter bus **arrival times** (HH:MM) and optional extra minutes (e.g., toilet break).")
st.write("The app will calculate departures spaced as close as possible to the chosen headway.")
...
if arrivals:
    try:
        ...
        for t in arrival_list:
            ...
            if len(parts) > 1 and parts[1].startswith("+"):
                try:
                    ...
                    st.warning(f"Invalid extra minutes in line: {t}")
        ...
        for i, t in enumerate(dep_minutes):
            ...
            ax.text(t, 0.25, f"{departures[i].strftime('%H:%M')}", 
        ...
        st.error("Invalid format. Please use HH:MM or HH:MM +Minutes (e.g., 10:15 +5).")
